import { Avator } from "types";
import { v4 as uuid } from "uuid";

import Avatar_01 from "assets/avatar/01.png";
import Avatar_02 from "assets/avatar/02.png";
import Avatar_03 from "assets/avatar/03.png";
import Avatar_04 from "assets/avatar/04.png";
import Avatar_05 from "assets/avatar/05.png";

const friends: Avator[] = [
  { id: uuid(), img: Avatar_01, name: "Jermey123" },
  { id: uuid(), img: Avatar_02, name: "Cindy 888" },
  { id: uuid(), img: Avatar_03, name: "Elli.H" },
  { id: uuid(), img: Avatar_04, name: "Bruce J" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
  { id: uuid(), img: Avatar_05, name: "KM" },
];

export default friends;
